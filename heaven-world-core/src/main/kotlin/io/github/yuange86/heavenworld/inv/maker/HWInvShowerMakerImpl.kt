package io.github.yuange86.heavenworld.inv.maker
 ...
 * @since 0.3.0
 */
class HWInvShowerMakerImpl(
    height: Int,
    title: Component
) : HWInvShowerMaker, HWInventoryImpl(height, title) {
    ...
    override fun onClick(listener: (x: Int, y: Int, event: InventoryClickEvent) -> Unit) {
        clickListener = { x, y, event -> event.isCancelled = true; listener(x, y, event) }
    }

    override fun onClickBottom(listener: (event: InventoryClickEvent) -> Unit) {
        clickBottomListener = { event ->
            when(event.click) {
                ClickType.SHIFT_LEFT, ClickType.SHIFT_RIGHT -> event.isCancelled = true
                else -> {}
            }
            listener(event)
        }
